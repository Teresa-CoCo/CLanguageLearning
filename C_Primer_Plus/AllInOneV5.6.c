...
#include<D:\Develop\CLanguageLearning-2\C_Primer_Plus\ninenine.h>
void main(void)
{
    int grate;
    int i =0;
    ...
    if ( i == 0)
    {
        printf("欢迎使用C语言工具箱V5.6版\n");
        Sleep(1000);
        printf("请输入你要使用的功能：\n1.转换长度工具  2.转换鞋码工具  3.BMI健康指数计算工具 4.多功能电表 5.幸运数字与运势 6.excel快查7.二进制十进制互转8.运行hyperv开机bat（beta v2）(需联网)9.扫雷游戏（鸣谢@Hannnine）10.链接ssh服务11.九九乘法表（新）");
        scanf("%d",&grate);
        if (grate == 1)
        {
            translong();
